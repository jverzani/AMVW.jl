...
abstract type FactorizationType{T, DoubleOrSingle, IsPencil, IsTwisted} end

## SingleShift(:SS) -- Q, D, U, Ut
## DoubleShift(:DS) -- Q, U, V, Ut, Vt, W
...
struct Real_DoubleShift_NoPencil_NotTwisted{T} <: FactorizationType{T, Val{:DS}, Val{:NoPencil}, Val{:NotTwisted}}
...
mutable struct ComplexRealSingleShift_NoPencil_NotTwisted{T} <: FactorizationType{T, Val{:DS}, Val{:NoPencil}, Val{:NotTwisted}}
...
function Base.convert{T}(::Type{ComplexRealSingleShift}, ps::Vector{Complex{T}}; ray=true)
    N = length(ps)
